...
extern uint32_t videoBuffer[1280*1024];
...
/*static*/ int keycode_table[256*2];						// X keycode -> Mac keycode translation table
...
*/
max_width = retrow ; max_height = retrow;
	width = max_width;
	height = max_height;
}
...
static bool has_mode(int type, int width, int height, int depth)
{
	...
	return 1;//SDL_VideoModeOK(width, height,
...
// Set mouse grab mode
static /*SDL_GrabMode*/ int set_grab_mode(int/*SDL_GrabMode */mode)
{
	//const SDL_VideoInfo *vi = SDL_GetVideoInfo();
	return 0;//(vi && vi->wm_available ? SDL_WM_GrabInput(mode) : SDL_GRAB_OFF);
}
...
public:
	...
	//SDL_Surface 
int *s;	// The surface we draw into
};
...
{
	set_video_mode(0/*display_type == DISPLAY_SCREEN ? SDL_FULLSCREEN : 0*/);
	int aligned_height = (VIDEO_MODE_Y + 15) & ~15;


	if (!use_vosf) {
		// Allocate memory for frame buffer
		the_buffer_size = (aligned_height + 2) * 4*retrow /*s->pitch*/;
...
void driver_base::adapt_to_video_mode() {
	...
	visualFormat.depth = 32;//sdl_depth_of_video_depth(VIDEO_MODE_DEPTH);
	visualFormat.Rmask = 0x00ff0000;//f->Rmask;
	visualFormat.Gmask = 0x0000ff00;//f->Gmask;
	visualFormat.Bmask = 0x000000ff;//f->Bmask;
	Screen_blitter_init(visualFormat, true, mac_depth_of_video_depth(VIDEO_MODE_DEPTH));

	// Load gray ramp to 8->16/32 expand map
	if (!IsDirectMode(mode))
		for (int i=0; i<256; i++)
			ExpandMap[i] = 0xff000000 | i<<16 | i<<8 |i<<0 ;//SDL_MapRGB(f, i, i, i);
		...
		while (fgets(line, sizeof(line) - 1, f)) {
			...
			if (video_driver_found) {
				...
				if (sscanf(line, "%d %d", &x_code, &mac_code) == 2)
					keycode_table[x_code & 0x1ff] = mac_code, n_keys++;
				else
					break;
			} else {
				// Search for SDL video driver string
				static const char sdl_str[] = "retro";
...
mode_str = "win/1280/1024";
	...
	screen_depth = 32;//SDL_GetVideoInfo()->vfmt->BitsPerPixel;
	int default_depth;
	switch (screen_depth) {
	case 8:
...
//	SDL_GetMouseState(&x, &y);
	...
	display_type = (display_type == DISPLAY_SCREEN) ? DISPLAY_WINDOW
		: DISPLAY_SCREEN;
	drv->set_video_mode(1/*display_type == DISPLAY_SCREEN ? SDL_FULLSCREEN : 0*/);
...
*/
	...
	if (!IsDirectMode(mode)) {
		for (int i=0; i<256; i++) {
			...
			ExpandMap[i] = 0xff000000 |pal[c*3+0]<<16| pal[c*3+1]<<8| pal[c*3+2]; //SDL_MapRGB(drv->s->format, pal[c*3+0], pal[c*3+1], pal[c*3+2]);
		}
...
static bool is_modifier_key(int /*SDL_KeyboardEvent*/const & e)
{
/*
	switch (e.keysym.sym) {
	case SDLK_NUMLOCK:
...
static bool is_ctrl_down(int /*SDL_keysym */const & ks)
{
...
static int kc_decode(int /*SDL_keysym*/ const & ks, bool key_down)
{
/*
	switch (ks.sym) {
...
static int event2keycode(int /*SDL_KeyboardEvent*/ const &ev, bool key_down)
{
	return kc_decode(0/*ev.keysym*/, key_down);
}
...
static void update_display_static0(driver_base *drv)
{
	...
	int bytes_per_pixel = VIDEO_MODE_ROW_BYTES / mode.x;
	uint8 *p, *p2;
...
i = j * bytes_per_row ;
int di=i* 4 ;
	Screen_blit((uint8 *)&videoBuffer[0]/*drv->s->pixels*/ + di, the_buffer + i, VIDEO_MODE_X);
}
#endif
#if 0
				for (j=y1; j<=y2; j++) {
					...
					memcpy(the_buffer_copy + i, the_buffer + i, bytes_per_pixel * wide);
					Screen_blit((uint8 *)&videoBuffer[0]/*drv->s->pixels*/ + dst_i, the_buffer + i, bytes_per_pixel * wide);
				}
#endif
/*
	// Refresh display
...
static void update_display_static(driver_base *drv)
{
	...
	if (high) {
		if ((int)VIDEO_MODE_DEPTH < VIDEO_DEPTH_8BIT) {
			...
			const int dst_bytes_per_row = 4*retrow;//drv->s->pitch;
...
*/
				...
				for (j = y1; j <= y2; j++) {
					...
					Screen_blit((uint8 *)&videoBuffer[0]/*drv->s->pixels*/ + di, the_buffer + si, wide / pixels_per_byte);
		...
		for (x = 0; x < VIDEO_MODE_X; x += N_PIXELS) {
			...
			for (int j = y; j < (y + h); j++) {
				...
				/*if (memcmp(&the_buffer[yb + xb], &the_buffer_copy[yb + xb], xs) != 0)*/ {
					...
					Screen_blit((uint8 *)&videoBuffer[0]/*drv->s->pixels*/ + dst_yb + xb, the_buffer + yb + xb, xs);
					dirty = true;
				}
			}
			if (dirty) {
...
extern int RLOOP,retrow,retroh,NEWGAME_FROM_OSD;
...
const VIDEO_MODE &mode = drv->mode;
if(oldx!=VIDEO_MODE_X || oldy!=VIDEO_MODE_Y){
printf("VIDEOREFRESH %dx%d %dx%d\n",VIDEO_MODE_X, VIDEO_MODE_Y,oldx,oldy);
oldx=VIDEO_MODE_X;
oldy= VIDEO_MODE_Y;
NEWGAME_FROM_OSD  = 1;
retrow=oldx;
